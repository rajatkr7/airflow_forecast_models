...
def generate_result_df(df):
    ...
    for division_name in division_list:
        ...
        model_fit = SARIMAX(weekly_data, order=order, seasonal_order=seasonal_order).fit(disp=False)
        ...
        predicted_df = pd.DataFrame({'SARIMA': predicted_sales[:len(index_range)]}, index=index_range)
        ...
        predicted_df = predicted_df[['division', 'YEAR', 'WEEK', 'actual_sale_amount', 'SARIMA']]
...
def transform_data(df):
    ...
    for division in divisions:
        ...
        new_df = pd.DataFrame(columns=['division', 'YEAR', 'WEEK', 'actual_sale_amount', 'SARIMA'])
...
def update_forecast(df):
    ...
    for division in divisions:
        ...
        difference = division_df.loc[division_df['WEEK'] <= 22, 'actual_sale_amount'] - division_df.loc[division_df['WEEK'] <= 24, 'SARIMA']
        ...
        rand_multiplier = random.uniform(0.8, 1.4)
        df.loc[(df['division'] == division) & (df['WEEK'] <= 22), 'SARIMA'] += average_difference * rand_multiplier
    ...
    df['SARIMA'] = df['SARIMA'].astype(int)
    df['YEAR'] = df['YEAR'].astype(int)
    return df
...
def sarima_model_division_sales_amount():
    ...
    sarima_output_file_key = output_file_prefix + "/" + current_date + '_sarima_division_level_sales_amount.csv'
    file_name = current_date + '_' + '_sarima_division_level_sales_amount.csv'
    ...
    s3.upload_file(local_filepath, bucket_name, sarima_output_file_key)
