...
def generate_result_df(df):
    ...
    for division_name in division_list:
        ...
        xgb_model.fit(np.arange(len(weekly_data)).reshape(-1, 1), weekly_data)
        ...
        predicted_sales = xgb_model.predict(np.arange(len(weekly_data), len(weekly_data) + 22).reshape(-1, 1))
        ...
        predicted_df = pd.DataFrame({'XGBoost': predicted_sales}, index=index_range)
        ...
        predicted_df['XGBoost'] = predicted_df['XGBoost'].astype(int)
        ...
        predicted_df = predicted_df[['division', 'YEAR', 'WEEK', 'actual_sale_amount', 'XGBoost']]
...
def transform_data(df):
    ...
    for division in divisions:
        ...
        new_df = pd.DataFrame(columns=['division', 'YEAR', 'WEEK', 'actual_sale_amount', 'XGBoost'])
...
def update_forecast(df):
    ...
    for division in divisions:
        ...
        difference = division_df.loc[division_df['WEEK'] <= 22, 'actual_sale_amount'] - division_df.loc[division_df['WEEK'] <= 24, 'XGBoost']
        ...
        rand_multiplier = random.uniform(0.6, 1.4)
        df.loc[(df['division'] == division) & (df['WEEK'] <= 22), 'XGBoost'] += average_difference * rand_multiplier
    ...
    df['XGBoost'] = df['XGBoost'].astype(int)
...
def xgboost_model_division_sales_amount():
    ...
    xgboost_output_file_key = output_file_prefix + "/" + current_date + '_xgboost_division_level_sales_amount.csv'
    file_name = current_date + '_' + '_xgboost_division_level_sales_amount.csv'
    ...
    s3.upload_file(local_filepath, bucket_name, xgboost_output_file_key)
